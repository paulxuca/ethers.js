...
export async function decryptCrowdsale(
  json: string,
...
export function decrypt(
  ...
  var getSigningKey = async function(
    key: Uint8Array,
    reject: (error?: Error) => void
  ) {
    var ciphertext = looseArrayify(searchPath(data, "crypto/ciphertext"));

    var computedMAC = hexlify(
      computeMAC(key.slice(16, 32), ciphertext)
    ).substring(2);
  ...
  return new Promise<SigningKey>(async function(resolve, reject) {
    ...
    if (kdf && typeof kdf === "string") {
      if (kdf.toLowerCase() === "scrypt") {
        ...
        scrypt(passwordBytes, salt, N, r, p, 64, async function(
          error,
          progress,
          key
        ) {
...
export function encrypt(
  ...
  return new Promise(function(resolve, reject) {
    ...
    libraries.scrypt(passwordBytes, salt, N, r, p, 64, function(
      error,
      progress,
      key
    ) {
