...
export class Wallet extends AbstractSigner {
  ...
  signMessage(message: Arrayish | string): Promise<string> {
    return Promise.resolve(
      joinSignature(this.signingKey.signDigest(hashMessage(message)))
    );
  }
  ...
  sendTransaction(
    ...
    return populateTransaction(transaction, this.provider, this.address).then(
      tx => {
  ...
  static createRandom(options?: any): Promise<Wallet> {
    ...
    if (options.extraEntropy) {
      entropy = arrayify(
        keccak256(concat([entropy, options.extraEntropy])).substring(0, 34)
      );
    }
  ...
  static async fromMnemonic(
    ...
    const privateKey = (await fromMnemonic(mnemonic, wordlist)).derivePath(
      path
    );
